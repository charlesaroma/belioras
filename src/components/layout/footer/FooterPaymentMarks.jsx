...
// The brands' own artwork, kept because a shopper scans for the logo they
// recognise and a wordmark does not carry the same reassurance. What changed
// is the frame around it: square corners and a hairline instead of a rounded,
// shadowed card, so six saturated chips sit inside the house style rather
// than on top of it.
  ...
  return (
    <li>
      <svg
        ...
        aria-label={label}
        viewBox="0 0 48 30"
        className="h-7 w-auto shadow-[0_0_0_1px_rgba(18,7,0,0.10)] sm:h-8"
      >
        <title>{label}</title>
        <rect width="48" height="30" fill={bg} />
        {children}
      </svg>
    </li>
  );
